...
async def test_chat_no_api_key(async_client: AsyncClient, auth_headers):
    ...
    mock_provider.generate.side_effect = ImportError("not available")
    with patch("app.services.ai.factory.get_ai_provider", return_value=mock_provider):
...
async def test_chat_with_history(async_client: AsyncClient, auth_headers):
    ...
    mock_provider = AsyncMock()
    mock_provider.name = "test_provider"
    mock_provider.generate.return_value = "Stay aware of your surroundings and use well-lit streets."
    with patch("app.services.ai.factory.get_ai_provider", return_value=mock_provider):
        response = await async_client.post("/api/v1/chat/message", json=payload, headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
